...
public class PlayManager {
    ...
    public PlayManager() {
        ...
        NEXTOMINO_Y = top_y + 500;


        currentMino = pickMino();
        currentMino.setXY(MINO_START_X, MINO_START_Y);
        nextMino = pickMino();
        nextMino.setXY(NEXTMINO_X, NEXTOMINO_Y);
    }
    ...
    public void update() {
        ...
        //checking if the current mino is active
        if(currentMino.active == false){
            //if mino is not active, put it into the staticBlocks
            //check if game is over
            if(currentMino.b[0].x == MINO_START_X && currentMino.b[0].y == MINO_START_Y){
                //meaning that the currentMino hasn't moved from the starting position, which indicates that there is no space. That only happens when game is over.
                gameOver = true;
            }
